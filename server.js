  ...
  db.query(sql, (err, results) => {
    ...
    inquirer
      ...
      .then((answer) => {
        ...
        db.promise()
          .query(sql, answer.departmentAdded, (err, result) => {
            if (err) throw err;
            console.log("Showing all departments");
            console.table(result);
          })
...
function addRole() {
  ...
  db.query(sql, (err, results) => {
    if (err) throw err;
    console.table(`Existing Departments Shown Below`);
    console.table(results);
    console.table(`REMINDER!!!
        \n Department must be created prior to adding role \n`);
    inquirer
      .prompt([
        {
          name: "title",
...
// IN ORDER TO ADD EMPLOYEE YOU MUST FIRST ADD ROLE AND DEPARTMENT
    ...
    inquirer
      .prompt([
        ...
        },
        {
          ...
          type: "input",
          message:
            "What is the manager id?(leave blank if employee is not a manager)",
        },
      ])
      .then((answer) => {
        let manager_id;
    ...
    inquirer
      .prompt([
        {
          ...
          type: "number",
          message:
            "Please enter the ID of the employee profile you would like to update.",
          validate: (value) => {
            if (value) {
              return true;
            } else {
      ...
      .then((answer) => {
        ...
        db.query(sql, (err) => {
          if (err) throw err;
          console.log(
            `No.${answer.employee_id} employee role has been updated`
          );
          startPrompt();
        });
      });
...
function fireEmployee() {
  ...
  db.query(sql, (err, results) => {
    ...
    inquirer
      ...
      .then((answer) => {
        const deletedID = answer.deleteEmployee;
        const sql = `DELETE from employee where id = ?`;
        db.query(sql, deletedID, (err, results) => {
          if (err) throw err;
          console.log(`${deletedID} is the weakest link. Goodbye`);
          startPrompt();
        });
      });
  });
